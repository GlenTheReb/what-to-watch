...
test('deck page loads cards and handles keep/pass', async ({ page }) => {
  ...
  // Mock the IP API call to prevent rate-limiting or hanging in tests
  await page.route('https://ipapi.co/country/', async route => {
    await route.fulfill({ body: 'US' });
  });
  ...
  await expect(page.getByText('Done swiping!')).toBeVisible();
